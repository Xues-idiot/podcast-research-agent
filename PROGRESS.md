...
#### API 速率限制
- 在 `src/echo/api/research.py` 添加内存限流器
- 限流策略: 每分钟最多10次请求
- 应用于 `/stream` 和 `/start` 端点
- 返回 429 状态码和 Retry-After 头
- 更新 deployment.md 安全注意事项

### 本轮完成
- [x] 添加 API 速率限制 (P1 item)
...
#### TranscriptPlayer 播放速度控制
- 添加 PLAYBACK_SPEEDS 数组 [0.5, 0.75, 1, 1.25, 1.5, 2]
- 添加 playbackSpeed 状态和速度选择器
- 用户可选择播放速度 (0.5x - 2x)
...
#### Export 组件增强
- 添加 generatePlainText 函数生成纯文本格式
- 添加 TXT 导出选项 (4列布局)
- 纯文本格式包含: 标题、摘要、要点、思维导图、问答对
...
#### TranscriptPlayer 复制功能
- 添加 copyToClipboard 函数
- 添加 copiedIndex 状态管理复制反馈
- 在悬停时显示复制按钮
- 复制成功显示 Check 图标 (2秒后恢复)
...
#### MindMap 展开/收起全部
- 添加 allExpanded 状态和 toggleAll 函数
- 在标题栏添加展开/收起全部按钮
- 用户可以一键展开或收起所有分支
...
#### SKILL.md 文档更新
- 添加 TranscriptPlayer 功能说明 (播放速度、复制、语言)
- 添加 MindMap 展开/收起功能
- 添加 KeyPoints 复制和重要性指示
- 添加 Export 4种格式说明
- 添加 API 速率限制说明
...
#### 回到顶部按钮
- 添加 showBackToTop 状态和 scroll 事件监听
- 滚动超过400px时显示按钮
- 使用 AnimatePresence 实现动画效果
...
#### URLInput 平台检测
- 添加 PLATFORM_PATTERNS 检测支持的平台
- 当URL不匹配任何已知平台时显示警告
- 支持B站、YouTube、抖音、微信、小红书、RSS
